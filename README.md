...
한 레코더에서 생성된 Blob을 처음부터 순서대로 보관하고, 저장할 때 완전한 연속 스트림에서 클릭 시점 이전의 최근 구간을 FFmpeg로 추출합니다. Blob 경계를 독립적인 미디어 조각으로 가정하지 않습니다. H.264는 영상 스트림을 복사하고, VP8/VP9 같은 비호환 코덱은 MP4로 재인코딩합니다. 메모리 상한에 도달하면 새 연속 버퍼를 시작합니다.
