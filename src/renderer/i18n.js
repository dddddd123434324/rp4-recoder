...
(function initI18n(RP4) {
  const ENGLISH = {
    ...
    'RP4 Recorder는 녹화 파일과 설정을 사용자의 장치에만 저장합니다.': 'RP4 Recorder stores recordings and settings only on your device.',
    ...
    '원본 프레임이 192MiB 안전 한도를 초과해 무압축 녹화를 시작할 수 없습니다.': 'The native frame exceeds the 192 MiB safety limit, so uncompressed recording cannot start.',
    ...
    '원본 프레임을 압축하지 않고 AVI로 저장합니다. 큰 화면에서는 프레임을 자동으로 건너뛸 수 있지만, 저장되는 프레임은 원본 해상도·무압축으로 유지됩니다.': 'Saves native frames to AVI without compression. On large displays frames may be skipped automatically, while every saved frame remains native-resolution and uncompressed.',
